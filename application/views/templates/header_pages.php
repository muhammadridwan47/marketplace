...
<head>
    ...
    <link rel="icon" type="image/png" sizes="32x32" href="<?= base_url('assets2/img/logo/') ?>gs.jpg">
...
<input type="hidden"  id="url"  value="<?= base_url() ?>">
  <!-- Navbar -->
  <nav class="navbar navbar-expand navbar-top d-none d-sm-block" style="z-index: 9999">
    ...
    <a href="<?= base_url('credit') ?>"  class="d-inline-block float-right mr-4 cart" style="color: #FF0000;font-size: 15px">
       Cr.<?= $user['saldo'] ?>
    </a>
  ...
  <nav class="navbar navbar-expand navbar-top d-block d-sm-none" style="height:37px;">
    ...
    <div class="row justify-content-center">
      <div class="col-6">
        <a href="<?= base_url('credit') ?>" class="d-block pt-1 text-right people-mobile" style="color: #FF0000;font-size: 15px;">
        <!-- <img  src="<?= base_url('assets2/') ?>img/logo/cart.svg" > 
        &nbsp;&nbsp; -->
         Cr.<?= $user['saldo'] ?></a>
      </div>
      <div class="col-6">
        <a href="<?= base_url('profile') ?>" class="d-inline-block ml-2 cart-mobile" style="color: #FF0000;font-size: 15px;">
        <img class="rounded-circle" src="<?= base_url('').$user['lokasi'].$user['image'] ?>" style="width: 23.68px;height:23.68px">
         <?= $user['name'] ?></a>
      </div>
    </div>
    ...
    <div class="container" >
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav text-uppercase">
          ...
          <li class="nav-item  menu d-none d-sm-inline ">
            <a class="nav-link" href="<?= base_url() ?>">HOME</a>
          </li>
          <?php if (count($this->db->get_where('product',['jenis' => 'Font','status' => 1])->result_array()) != 0): ?>
          <li class="nav-item  menu">
            <a class="nav-link" href="<?= base_url('product/fonts') ?>">FONTS</a>
          </li>
          <?php endif ?>
          <?php if (count($this->db->get_where('product',['jenis' => 'Graphic','status' => 1])->result_array()) != 0): ?>
            <li class="nav-item  menu">
              <a class="nav-link" href="<?= base_url('product/graphics') ?>">GRAPHICS</a>
            </li>
          <?php endif ?>
          
          <?php if (count($this->db->get_where('product',['jenis' => 'Photo','status' => 1])->result_array()) != 0): ?>
        ...
        <?php else: ?>
        <a  class="nav-link auth text-white ml-auto" style="cursor: pointer;"><span class="registermenu" data-dismiss="modal" data-toggle="modal" data-target="#registerModal">Register |</span> <span id="loginmenu" data-toggle="modal" data-target="#loginModal">Login</span></a>
        <?php endif ?>
      </div>
    </div>
  </nav>
