   ...
   if ($this->db->get_where('user',['email' => $this->session->userdata('email')])->row_array()) {
    ...
    public function render($id,$size=null)
    {
        ...
        if ($this->db->get_where('sub_product',['id_barang' => $id,'ektensi' => 'OTF/'])->result_array()) {
         ...
         $path = FCPATH.'renderingfont/'.$this->input->ip_address();
         if (!file_exists($path)) {
            mkdir($path,0755,true);
        } 

        $j = 1;
        $files = glob('renderingfont/'.$this->input->ip_address().'/*'); // Ambil semua file yang ada dalam folder
        ...
        foreach ($data as $da ) {        
            ...
            imagepng($image,'renderingfont/'.$this->input->ip_address().'/'.$j.'I'.$random.'.png');
            $j++;
        }   
        

        if (glob('renderingfont/'.$this->input->ip_address().'/*')) {
            // $this->output
