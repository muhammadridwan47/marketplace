   ...
   if ($this->db->get_where('user',['email' => $this->session->userdata('email')])->row_array()) {
    ...
    public function colection($id,$status=null)
    {
        ...
        if ($this->db->get_where('colection',['id_barang'=> $id,'add' => $this->session->userdata('email') ])->row_array()) {
            ...
            $rating = $pemilik['rating'] + (1 * 100000);
    ...
    public function render($id,$size=null)
    {
        ...
        if ($this->db->get_where('sub_product',['id_barang' => $id,'ektensi' => 'OTF/'])->result_array()) {
         ...
         $path = FCPATH.'renderingfont/'.$this->session->userdata('render').'/';
         if (!file_exists($path)) {
            mkdir($path,0755,true);
        } 

        $j = 1;
        $files = glob('renderingfont/'.$this->session->userdata('render').'/*'); // Ambil semua file yang ada dalam folder
        ...
        foreach ($data as $da ) {        
            ...
            imagepng($image,'renderingfont/'.$this->session->userdata('render').'/'.$j.'I'.$random.'.png');
            $j++;
        }   
        

        if (glob('renderingfont/'.$this->session->userdata('render').'/*')) {
            // $this->output
            ...
            if ($this->input->is_ajax_request()) {
                if ($_SERVER['REQUEST_METHOD'] == 'POST')
                {
                  ...
                  $csrf_token =  $this->security->get_csrf_hash();
                   ...
                   if ($barang = $this->db->get_where('product',['daily_deal' => '1'])->row_array()) {
                   


                    
            
                        if (time() - 1591761533 > (60 * 60 * 24)) {
